...
describe('FateRewardControllerV3.setRewardSchedule', () => {
  ...
  describe('setRewardSchedule', async () => {
    it('reverted cases:', async () => {
      // when trying to set with not-owner
      await expect(
        fateRewardControllerV3.connect(this.dev).setRewardSchedule(rewardSchedule.address)
      ).to.be.revertedWith('Ownable: caller is not the owner')
    })
    it('success cases:', async () => {
      await expect(fateRewardControllerV3.setRewardSchedule(rewardSchedule.address))
        .to.emit(fateRewardControllerV3, 'RewardScheduleSet')
